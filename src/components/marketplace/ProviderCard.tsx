...
const ProviderCard = ({ provider }: ProviderCardProps) => {
  ...
  const renderLocation = () => {
    ...
    } else if (provider.location && typeof provider.location === 'object') {
      return `${provider.location.address}, ${provider.location.city}`;
    }
    return '';
  };
    ...
    <div className="bg-white rounded-xl overflow-hidden border border-border shadow-sm hover:shadow-md transition-all group animate-fade-in">
      <div className="flex flex-col md:flex-row">
        <div className="relative md:w-1/3 h-48 md:h-auto overflow-hidden">
          ...
          {'verified' in provider && provider.verified && (
          ...
          {'availableToday' in provider && provider.availableToday && (
        ...
        <div className="p-6 md:w-2/3">
          ...
          <div className="flex justify-between items-center mt-4">
            <div className="text-sm">
              <span className="font-medium">{t('services.available')}:</span>{' '}
              {'availability' in provider && Array.isArray(provider.availability) 
